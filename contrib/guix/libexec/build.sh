...
# Setup the directory where our PRCYcoin build for HOST will occur
...
if [ -z "$(find "${OUTDIR}/src" -name 'prcycoin-*.tar.gz')" ]; then
    ./autogen.sh
    env CONFIG_SITE="${BASEPREFIX}/${HOST}/share/config.site" ./configure --prefix=/
    make dist GZIP_ENV='-9n' ${V:+V=1}
    mkdir -p "${OUTDIR}/src"
    mv "$(find "${PWD}" -name 'prcycoin-*.tar.gz')" "${OUTDIR}/src/"
fi

# Determine the full path to our source tarball
SOURCEDIST="$(find "${OUTDIR}/src" -name 'prcycoin-*.tar.gz')"
# Determine our distribution name (e.g. prcycoin-0.18.0)
...
(
    ...
    # Setup the directory where our PRCYcoin build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built PRCYcoin to $INSTALLPATH
...
case "$HOST" in
    *mingw*)
        ...
        (
            cd ./windeploy
            mkdir unsigned
            cp --target-directory=unsigned/ "$OUTDIR"/prcycoin-*-setup-unsigned.exe
            find . -print0 \
                | sort --zero-terminated \
                | tar --create --no-recursion --mode='u+rw,go+r-w,a+X' --null --files-from=- \
