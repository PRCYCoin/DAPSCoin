...
void initalizeMapParamBootstrap() {
  ...
  bootFile.URL = "http://bootstrap.aprcycoin.com/PRCY-bootstrap.tar.gz";
  bootFile.verified = false;
  bootFile.path = GetDataDir() / "PRCY-bootstrap.tar.gz";
  bootFile.dlnow = 0;
  bootFile.dltotal = 0;
...
void getHttpsJson(std::string url, JsonDownload* reply, int headerType)
{
    ...
    if(curl) {
        ...
        if(CURLE_OK == res) {
            ...
            if((CURLE_OK == res) && ct) {
                reply->response = std::move(response_string);
                reply->failed = false;
                reply->complete = true;
            }
        } else {
...
bool getBootstrap() {
    ...
    for (std::map<std::string, ParamFile>::iterator it = mapParams.begin(); it != mapParams.end(); ++it) {
        ...
        if (fs::exists(it->second.path.string())) {
            fs::remove(it->second.path.string());
        }
    }
    return dlsuccess;
}
